			...
			<Fragment>
				<figure className="w-full flex flex-col justify-center items-center">
					<div className="relative w-2/4 md:w-1/4 aspect-square rounded-lg overflow-hidden">
						...
						<div className="absolute px-2 py-1 flex flex-col justify-center items-center gap-1 top-1 left-1 rounded-md bg-black/50">
					...
					<figcaption className="mt-4 text-center">
						<h2 className="flex gap-2 justify-center items-center text-2xl text-neutral-100 font-bold mb-1">{specificArtistDetails.name}</h2>
						<p className="text-sm text-neutral-300 font-semibold">{specificArtistDetails.dominantType} - {specificArtistDetails.dominantLanguage} {specificArtistDetails.dob ? `- ${specificArtistDetails.dob}` : ""}</p>
						<span className="leading-none text-sm text-neutral-500">{specificArtistDetails.followerCount} followers</span>
					</figcaption>
				</figure>

				<div className="w-full max-w-lg mt-8 mb-4 mx-auto">
					<h3 className="text-lg font-semibold text-neutral-100">More about {specificArtistDetails.name}</h3>
					<div className="mt-3">
						{specificArtistDetails.topSongs && <Fragment>
							<h4 className="text-base text-neutral-200 font-semibold mt-4 mb-2">Top Songs</h4>
						...
						{specificArtistDetails.availableLanguages && <Fragment>
							...
							<div className="w-full flex gap-2 flex-wrap mb-8">
								{specificArtistDetails.availableLanguages.length ? specificArtistDetails.availableLanguages.map((language, index) => (
									<span className="px-1.5 py-0.5 border border-yellow-400 rounded-md text-sm font-semibold text-yellow-400" key={index}>{language.charAt(0).toUpperCase() + language.slice(1)}</span>
								)) : <p className="text-sm text-neutral-400">No language available!</p>}
							</div>
						</Fragment>}
						{specificArtistDetails.similarArtists && <Fragment>
							<h4 className="text-base text-neutral-200 font-semibold mt-4 mb-2">Similar Artists</h4>
