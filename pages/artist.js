...
import LoadingSpecificArtist from '../components/loadings/loadingSpecificArtist';
			...
			<section className="animate-fade-in-up min-h-0 w-full px-4 md:px-8 lg:px-12 pt-4">
				<figure className="w-full flex flex-col justify-center items-center">
					...
					<figcaption className="mt-4 text-center">
						<h2 className="flex gap-2 justify-center items-center text-2xl text-neutral-800 font-medium mb-1">{specificArtistDetails.name}</h2>
						<p className="text-sm text-neutral-600 font-normal">{specificArtistDetails.dominantType} - {specificArtistDetails.dominantLanguage} {specificArtistDetails.dob ? `- ${specificArtistDetails.dob}` : ""}</p>
				...
				<div className="w-full max-w-lg mt-8 mb-4 mx-auto">
					<h3 className="text-lg font-normal text-neutral-600">More about {specificArtistDetails.name}</h3>
					<div className="mt-3">
						{specificArtistDetails.topSongs && <Fragment>
							<h4 className="text-base text-neutral-800 font-normal mt-4 mb-2">Top Songs</h4>
						...
						<h4 className="text-base text-neutral-800 font-normal mt-4 mb-2">External Links</h4>
						...
						{specificArtistDetails.availableLanguages && <Fragment>
							<h4 className="text-base text-neutral-800 font-normal mt-4 mb-2">Available Languages</h4>
							<div className="w-full flex gap-2 flex-wrap mb-8">
								{specificArtistDetails.availableLanguages.length ? specificArtistDetails.availableLanguages.map((language, index) => (
									<span className="px-1.5 py-0.5 bg-yellow-300 rounded-md shadow-md text-sm font-normal text-neutral-600" key={index}>{language.charAt(0).toUpperCase() + language.slice(1)}</span>
								)) : <p className="text-sm text-neutral-400">No language available!</p>}
							</div>
						</Fragment>}
						{specificArtistDetails.similarArtists && <Fragment>
							<h4 className="text-base text-neutral-800 font-normal mt-4 mb-2">Similar Artists</h4>
