	...
	render() {
		return (
			<section className="fade_in_up min-h-0 grow w-full overflow-y-auto px-4 md:px-8 lg:px-12 pt-4 pb-[65px]">
				...
				{
					[
						{ icon: 'fa-user', label: 'About', path: '/about', color: 'from-cyan-500 to-cyan-300' },
						{ icon: 'fa-search', label: 'Search', path: '/search', color: 'from-yellow-500 to-yellow-300' },
						{ icon: 'fa-bookmark', label: 'Saved', path: '/saved', color: 'from-blue-500 to-blue-300' },
						{ icon: 'fa-cogs', label: 'Settings', path: '/settings', color: 'from-rose-500 to-rose-300' }
					].map(({ icon, label, path, color }) => (
						<div key={label} className={`flex-shrink-0 w-28 h-28 rounded-md bg-gradient-to-tr ${color}`}>
							<Link to={path} className="w-full h-full flex flex-col gap-1 justify-end items-start px-3 py-2">
								<i className={`fas ${icon} fa-md text-white`}></i>
				...
				{this.state.loading ? (
					...
					<section className="fade_in w-full max-w-md mx-auto flex flex-col justify-start items-center mt-6 space-y-2">
