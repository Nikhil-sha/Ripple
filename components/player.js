...
class Player extends Component {
	...
	return (
		<div
			className={`${
				isExpanded ? "h-full md:h-96" : ""
			} max-w-md w-full absolute bottom-0 left-0 z-40 bg-white rounded-t-xl border-x border-t border-gray-200 overflow-auto`}
			aria-live="polite"
		>
			...
			>
				<div
					className="absolute bottom-0 -z-10 left-0 h-full bg-sky-200/25"
					...
					<img
						src={
							currentTrack
								...
								: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSjzAhqWfbijrFBGnEPl8zPR9lvnAocwAXzjQ&s"
						}
				...
				<div className="min-w-0 grow">
					<h2 className="text-md font-bold leading-tight truncate" aria-live="assertive">
						{currentTrack ? currentTrack.name : "No Track"}
					</h2>
					<p className="text-xs text-gray-400 truncate" aria-hidden="true">
			...
			<div className={`${isExpanded ? "" : "hidden"} w-full p-2`} aria-hidden={!isExpanded}>
				<button
					className="p-1 sticky block ml-auto"
					onClick={this.handleExpand}
				...
				<div className="w-full">
					<div
						className={`w-52 h-52 mx-auto rounded-xl shadow-xl overflow-hidden mb-4 ${
						...
						<div className="inline-flex flex-row justify-center items-center gap-4 mt-2">
							...
							>
								<i
									className={`fas ${
										repeatMode === "single"
											? "fa-repeat text-blue-500"
											: repeatMode === "playlist"
											? "fa-rotate text-blue-500"
											: "fa-repeat text-gray-800"
									}`}
								/>
							</button>
							<button
							...
							>
								<i
									className={`fas ${isShuffling ? "fa-random text-blue-500" : "fa-random"}`}
								/>
							</button>
						</div>
					</div>
					...
					<ul className="w-full">
						{this.context.playList.length ? (
							this.context.playList.map((track, index) => (
								<li
									key={index}
									className="px-3 py-2 flex flex-row items-center gap-3 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer"
								...
								>
									...
									<div className="grow flex flex-col">
										...
										<span className="text-xs text-gray-500 leading-none truncate" aria-hidden="true">
											{track.artist}
										</span>
									</div>
									<span
										className="p-1 fas fa-times"
										onClick={(e) => {
											e.stopPropagation();
