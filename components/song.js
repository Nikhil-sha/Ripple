...
class Song extends Component {
	...
	render() {
		const { songId, name, artist, coverSm, tailwind, option } = this.props;

		return (
			<div className={`w-full py-2 px-3 flex items-center gap-3 rounded-md hover:bg-neutral-200/50 transition-colors duration-200 ${tailwind || ""}`}>
				<Link to={`/song/${songId}`} className="min-w-0 grow flex items-center gap-3">
					...
					<div className="flex-1 min-w-0">
						<h2 className="text-sm font-normal text-neutral-800 truncate mb-0.5">{renderText(name)}</h2>
						<p className="text-xs text-neutral-600 truncate">{renderText(artist)}</p>
					</div>
				</Link>
				<button 
				...
				>
					<i className="fa-solid fa-play pl-0.5 text-neutral-700 group-hover:text-neutral-100"></i>
				</button>
				{option === "save" ?
					<button 
					...
					>
						<i className="fa-solid fa-heart text-neutral-100 group-hover:text-yellow-400"></i>
					...
					>
						<i className="fa-solid fa-times text-neutral-100 group-hover:text-red-400"></i>
					</button> : ''
				}
			</div>
		);
